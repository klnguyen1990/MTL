...
def calcul_loss(model, l1_fc1, l2_fc1, l1_fc1_df, l2_fc1_df, 
                prob, labels, prob_diffuse, labels_diffuse, seg, labels_seg, decode, inputs, score_weight) : 
    ...
    loss = loss_classif * score_weight + loss_classif_diffuse + loss_seg + loss_reconst
    
    return loss, loss_seg, loss_classif, loss_classif_diffuse 
...
def train(fold, model, nb_epoch, score_weight, l1_fc1, l2_fc1, l1_fc1_df, l2_fc1_df, dim, spacing, scale, sigma,
    ...
    [train_loss,val_loss,train_loss_seg,val_loss_seg,train_loss_class,val_loss_class,
            train_loss_class_diffuse,val_loss_class_diffuse] = [np.zeros(nb_epoch) for i in range(8)]
    ...
    for epoch in range(nb_epoch) :
        ...
        for data in trainloader:
            ...
            optimizer.zero_grad() 
            prob, prob_diffuse, reconst, seg = model(image)
            loss, loss_seg, loss_classif, loss_classif_diffuse = calcul_loss(model, l1_fc1, l2_fc1, l1_fc1_df, l2_fc1_df, 
                                            prob, labels, prob_diffuse,labels_diffuse, seg, labels_seg, reconst, image, score_weight)
            loss.backward()
            optimizer.step() 
        ...
        subfigs[0].subplots(3, 3, sharex=True)

        plot_graphic_diffuse(fold, nb_train, train_c1, train_c2, train_c3, nb_val, val_c1, val_c2, val_c3, dim, spacing, scale, sigma, nb_parametres, score_weight, 
        ...
        #if train_micro_f1_score[epoch] > 0.98 : break
...
def evaluation(model, list_patient, label_classe,label_classe_diffuse, label_classe_dfo, scale, sigma, dim, spacing, num_workers, dir_p_1) : 
    ...
    for i, data in enumerate(valloader) :
        ...
        proba, proba_diffuse, reconst, seg = model(image)
    ...
    np.save(dir_p_1+'/all_image_seg.npy', all_image_seg)
